...
Projet : mettre en entrée la position xo"""
...
nbneurone = [2,4,2] #la couche intermédiaire est encore à déterminer
...
def fitness(poids, nbneurone, nbit, dt): #lance une sinulation, détermine le fitness selon les règles en haut, nbit : nombre d'itérations, dt : intervalle de temps entre deux valeur, nbtest : nombre de test avec une valeur initiale de G différente
    ...
    for j in T:
        ...
        for i in range(nbit):
            ...
            sortie[0] = [G,dG]
            ddxo = sortiefunc(reseau(poids, sortie, nbneurone)[-1]) #valeur à t = i*dt de ddxo
...
def apprentissage(nbneurone, population = 50, fitvoulu = 100, generationmax = 300, nbit = 100, dt = 1/10): #generationmax = val de la gene max pr eviter boucle infinie si jamais EQM non atteint
...
def montre(poids, nbneurone, nbit, dt,G0):
    ...
    for i in range(nbit):
        sortie = gensortie(nbneurone)
        sortie[0] = [G,dG]
        DDXO.append(sortiefunc(reseau(poids, sortie, nbneurone)[-1]))
        (G,dG) = next(G,dG,DDXO[-1],1,dt)
    return DDXO
    
def k(p,nbneurone,G0):
    DDXO = montre(p,nbneurone,100,0.1,G0)
